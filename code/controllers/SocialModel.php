...
use FieldList;
use Member;
use Modular\Controllers\Graph\Node;
 ...
 */
class SocialModelController extends Node implements SocialModelControllerInterface, FormControllerInterface {
	use json;
	use reflection;
	use requirements;
	...
	/**
	 * Return the 'Action' or EdgeType which matches the request.
	 * @return SocialEdgeType
	 */
	public function getAction() {
		$request = $this->getRequest();
		$actionName = current(array_reverse(array_filter(explode('/', $request->getURL(false)))));
		if ($actionName = $this->extend('provideMode', $actionName)) {
			return SocialEdgeType::get_by_action_name($actionName);
		}
	}
...
JS
	 ...
	 *
	 * @param bool $stripNamespace return the 'terminal' part of the class name without namespace
	 * @return string
	 */
	public function getModelClass($stripNamespace = false) {
		$modelClass = static::config()->get('model_class') ?: substr($this->class, 0, strpos($this->class, '_'));
		return $stripNamespace ? static::strip_namespace($modelClass) : $modelClass;
	}
