...
//              [$this()->beforeRegister($request, $model, $mode)]
	...
	public function afterRegister(SS_HTTPRequest $request, DataObject $model, $mode) {
		...
		if (!$result) {
			...
			/** @var Member|ConfirmableExtension $model */
			if ($model instanceof Member) {
				$member = $model;
				$member->write();
				...
				MemberMemberRelationship::make($member, $member, 'REG');
			...
			if ($model instanceof Organisation) {
				...
				if ($request->postVar(OrganisationTypesChooser::IDFieldName)) {
					$subTypes = $request->postVar(OrganisationTypesChooser::IDFieldName);
					if (count($subTypes)) {
				...
				Approveable::disable();
				MemberOrganisationRelationship::make($member, $organisation, 'REG');
				Approveable::enable();
	...
	/**
	 * Adds an OrganisationTypesChooser to form, we can't just put this in the fields_for_mode['register'] collection as
	...
	public function updateFieldsForMode(DataObject $model, FieldList $fields, $mode, &$requiredFields = []) {
		if ($mode === Registerable::Action) {
			list($fieldName, $fieldLabel) = OrganisationTypesChooser::get_field_config();

			if (!$chooserField = $fields->dataFieldByName($fieldName)) {
				$chooserField = new OrganisationTypesChooser();
